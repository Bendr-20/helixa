/**
 * Helixa Aura Generator v3.0 - TypeScript
 * QR-aesthetic faces with 10 eye types, 10 mouth types,
 * rarity tiers (Common → Legendary) with gradient/chrome/glow effects.
...
export interface AgentData {
  ...
  humor?: string;
  values?: string;
  // V2 narrative fields (influence visuals)
  origin?: string;
  mission?: string;
  credScore?: number;
}
...
const FRAMEWORK_SHAPES: Record<string, { faceRadius: number; baseHue: number }> = {
...
const ALIGNMENT_SHIFT: Record<string, number> = {
...
// ============ RARITY SYSTEM ============
function getRarity(points: number, mutationCount: number): RarityType {
...
const RARITY_COLORS: Record<RarityType, { label: string; borderOpacity: number }> = {
...
function getEyeType(specialization: string, rng: () => number, rarity: RarityType): number {
...
function drawEye(grid: number[][], G: number, ex: number, ey: number, eyeR: number, eyeType: number): void {
  switch (eyeType) {
    case 0: // round
...
function getMouthType(commStyle: string, rng: () => number, rarity: RarityType): number {
...
function drawMouth(grid: number[][], G: number, cx: number, mouthY: number, mW: number, mouthType: number): void {
  switch (mouthType) {
    case 0: // line
...
function buildPalette(address: string, riskTolerance: number, alignment: string): ColorPalette {
...
export function generateAura(agentData: AgentData, size = 400): string {
  const G = 25;
  const cs = size / G;
  ...
  // Derive V1 visual params from V2 personality when V1 fields are absent
  ...
  // Map humor → temperament influence
  const humorMap: Record<string, string> = {
    'dry': 'analytical', 'sarcastic': 'reactive', 'dark': 'volatile',
    'absurd': 'creative', 'wholesome': 'empathetic', 'wit': 'analytical',
  };
  ...
  // Derive alignment from autonomy + risk
  const alignmentFromV2 = riskTolerance >= 7
    ? (autonomyLevel >= 7 ? 'chaotic-good' : 'neutral-good')
    : riskTolerance <= 3
    ? (autonomyLevel >= 7 ? 'lawful-neutral' : 'lawful-good')
    : 'true-neutral';
  const alignment = agentData.alignment || alignmentFromV2;

  // Derive specialization from quirks/values/mission
  const specHints: Record<string, string> = {
    'build': 'engineer', 'ship': 'engineer', 'code': 'engineer',
    'research': 'researcher', 'analyze': 'researcher', 'data': 'researcher',
    'trade': 'trader', 'defi': 'trader', 'yield': 'trader',
    'art': 'artist', 'creat': 'artist', 'design': 'artist',
    'guard': 'guardian', 'secur': 'guardian', 'protect': 'guardian',
    'social': 'diplomat', 'connect': 'diplomat', 'communit': 'diplomat',
  };
  ...
  // V2 fields contribute to seed for more unique visuals
  ...
  // QR Corners
  function qr(sx: number, sy: number) {
    for (let y = 0; y < 7; y++)
      for (let x = 0; x < 7; x++)
  ...
  function cellColor(cellType: number): string {
    if (rarity === 'legendary') {
  ...
  if (rarity !== 'common') {
    ...
    const badgeColor = badgeColors[rarity as keyof typeof badgeColors];
...
export function getAuraRarity(points: number, mutationCount: number): RarityType {
