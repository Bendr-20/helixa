...
// ─── Cred Score Breakdown Panel ──────────────────────────────────
function CredBreakdownPanel({ tokenId }: { tokenId: number }) {
  const [data, setData] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch(`${API}/agent/${tokenId}/cred-breakdown`)
      .then(r => r.ok ? r.json() : null)
      .then(d => { setData(d); setLoading(false); })
      .catch(() => setLoading(false));
  }, [tokenId]);

  if (loading) return <div style={s.editCard}><div style={{ color: '#888' }}>Loading cred breakdown...</div></div>;
  if (!data) return null;

  const barBg = 'rgba(255,255,255,0.06)';
  const tierColors: Record<string, string> = { PREFERRED: '#b490ff', PRIME: '#33ff33', QUALIFIED: '#ffd93d', MARGINAL: '#ffaa00', JUNK: '#ff4444' };
  const tierColor = tierColors[data.tier?.tier] || '#888';

  return (
    <div style={s.editCard}>
      <h2 style={s.editHeading}>Cred Score</h2>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px', marginBottom: '16px' }}>
        <span style={{ fontSize: '2.5rem', fontWeight: 700, fontFamily: "'Orbitron', sans-serif", color: tierColor }}>{data.credScore}</span>
        <span style={{ fontSize: '1rem', fontWeight: 600, color: tierColor }}>{data.tier?.label}</span>
        <span style={{ fontSize: '0.8rem', color: '#666' }}>/ 100</span>
      </div>

      {Object.entries(data.breakdown || {}).map(([key, comp]: [string, any]) => {
        const pct = Math.round((comp.weightedScore / comp.maxWeightedScore) * 100) || 0;
        const isMaxed = pct >= 95;
        return (
          <div key={key} style={{ marginBottom: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', marginBottom: '4px' }}>
              <span style={{ color: isMaxed ? '#6eecd8' : '#ccc' }}>{comp.label} {isMaxed && '✓'}</span>
              <span style={{ color: '#888' }}>{comp.weightedScore} / {comp.maxWeightedScore}</span>
            </div>
            <div style={{ height: '6px', borderRadius: '3px', background: barBg, overflow: 'hidden' }}>
              <div style={{
                height: '100%', borderRadius: '3px', width: `${pct}%`,
                background: isMaxed ? '#6eecd8' : pct > 50 ? '#b490ff' : pct > 0 ? '#ffaa00' : 'transparent',
      ...
      {data.recommendations && data.recommendations.length > 0 && (
        <div style={{ marginTop: '20px', padding: '14px', background: 'rgba(180,144,255,0.06)', borderRadius: '8px', border: '1px solid rgba(180,144,255,0.15)' }}>
          <div style={{ fontSize: '0.85rem', fontWeight: 600, color: '#b490ff', marginBottom: '10px' }}>How to increase your score:</div>
          {data.recommendations.map((rec: any, i: number) => (
            <div key={i} style={{ fontSize: '0.8rem', color: '#ccc', marginBottom: '6px', display: 'flex', gap: '8px' }}>
              <span style={{ color: rec.priority === 'HIGH' ? '#6eecd8' : rec.priority === 'MEDIUM' ? '#ffd93d' : '#888', fontWeight: 600, minWidth: '20px' }}>→</span>
              <span>{rec.action} <span style={{ color: '#888' }}>({rec.impact})</span></span>
            </div>
          ))}
        </div>
      )}
...
// ─── Soulbound Panel ─────────────────────────────────────────────
function SoulboundPanel({ tokenId, meta }: { tokenId: number; meta: AgentMeta | null }) {
  ...
  useEffect(() => {
    if (meta?.attributes) {
      const sb = meta.attributes.find(a => a.trait_type === 'Soulbound');
      setIsSoulbound(sb?.value === 'Yes');
    }
  }, [meta]);

  async function makeSoulbound() {
    if (!window.ethereum) { setStatus({ msg: 'No wallet found.', type: 'error' }); return; }
    if (!confirm('This is PERMANENT. Once soulbound, this agent can never be transferred. Are you sure?')) return;

    setLocking(true);
    setStatus({ msg: 'Requesting wallet...', type: 'info' });
    ...
    try {
      ...
      setStatus({ msg: 'Confirm the transaction in your wallet...', type: 'info' });
      const tx = await contract.makeSoulbound(tokenId);
      setStatus({ msg: 'Transaction submitted, waiting for confirmation...', type: 'info' });
      await tx.wait();
      setIsSoulbound(true);
      setStatus({ msg: 'Agent is now soulbound! +5 Cred Score points.', type: 'success' });
    } catch (e: any) {
      if (e.code === 4001 || e.code === 'ACTION_REJECTED') {
        setStatus({ msg: 'Transaction rejected.', type: 'error' });
      } else {
        setStatus({ msg: `Error: ${e.reason || e.message}`, type: 'error' });
      }
    }
    setLocking(false);
  }
  ...
  if (isSoulbound) {
    return (
      <div style={s.editCard}>
        <h2 style={s.editHeading}>Soulbound Status</h2>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(110,236,216,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6eecd8" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>
          </div>
          <div>
            <div style={{ color: '#6eecd8', fontWeight: 600, fontSize: '0.9rem' }}>Soulbound</div>
            <div style={{ color: '#888', fontSize: '0.8rem' }}>This identity is permanently locked to its wallet</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={s.editCard}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
        <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'rgba(180,144,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#b490ff" strokeWidth="2"><path d="M8 7h12m0 0l-4-4m4 4l-4 4M16 17H4m0 0l4 4m-4-4l4-4"/></svg>
        </div>
        <div>
...
function EditPanel({ tokenId, onBack }: { tokenId: number; onBack: () => void }) {
  ...
  return (
    <div style={{ maxWidth: '700px', margin: '0 auto' }}>
      ...
      <CredBreakdownPanel tokenId={tokenId} />
