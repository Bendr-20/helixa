...
### Core Functions
- **`lockSoulVersion(tokenId, soulHash)`** — increments version counter, stores hash + timestamp as a new immutable entry
- **`getFullSoulHistory(tokenId)`** — returns the complete chain of hashes and timestamps
- **`lockSoul(tokenId, soulHash)`** — backward-compatible single lock (reverts if already locked)
- **`soulHash(tokenId)`** — returns the latest version's hash

### Design
- **1-hour cooldown** between version locks (anti-spam)
- **Auth:** agent wallet or token owner
- **Events:** `SoulVersionLocked(tokenId, version, soulHash, timestamp)` + legacy `SoulLocked`
- Each version is immutable once written — full provenance chain

### Why Versioned Locking?
Agents evolve. A single soul hash captures one moment; Chain of Identity captures the full arc — every personality update, every mutation, every evolution — as an immutable onchain history.
