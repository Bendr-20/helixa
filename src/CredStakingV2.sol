...
contract CredStakingV2 is ReentrancyGuard, Ownable {
    ...
    /// @notice Emergency withdraw all reward tokens (undistributed) to owner.
    ///         Only callable by owner. Does NOT touch staker principal.
    function emergencyWithdrawRewards() external onlyOwner {
        uint256 contractBalance = credToken.balanceOf(address(this));
        // Staker principal that must stay
        uint256 stakerFunds = totalStaked;
        // Also count vouch deposits
        // Whatever is above stakerFunds is reward pool
        require(contractBalance > stakerFunds, "no excess to withdraw");
        uint256 excess = contractBalance - stakerFunds;
        credToken.safeTransfer(owner(), excess);
        // Reset reward accounting
        rewardPerEffectiveStake = 0;
        emit EmergencyWithdraw(excess);
    }

    /// @notice Emergency withdraw ANY ERC-20 token sent to this contract by mistake.
    ///         Cannot withdraw $CRED below staker principal.
    function emergencyRescueToken(address token, uint256 amount) external onlyOwner {
        if (token == address(credToken)) {
            uint256 contractBalance = credToken.balanceOf(address(this));
            require(contractBalance - amount >= totalStaked, "would take staker funds");
        }
